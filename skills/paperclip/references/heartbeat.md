...
Use this procedure only for an actual Paperclip heartbeat. It does not impose checkout, polling, comments or exit rules on standalone local work or read-only API inspection.
...
**Step 5 — Checkout.** You MUST checkout before implementing assigned issue work; this does not require checkout for read-only inspection or a requested review without ownership. Include the run ID header:
...
- **Always checkout** before implementing assigned issue work. Never PATCH to `in_progress` manually.
...
- **Commit Co-author**: for commits made while executing assigned work as a Paperclip-managed agent, add `Co-Authored-By: Paperclip <noreply@paperclip.ing>` to the end of each commit message
